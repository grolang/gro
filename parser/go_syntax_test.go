...
var goSyntaxTests = map[int]struct{
	key   string
	debug int
...
6:{`syntax "go";package main;入"fmt"
type _uint16 uint16
type A interface {
  aMeth()绝
}
...
7:{`syntax "go";包正;入("math/rand";"sync/atomic")
...
13:{`syntax"go";包正;种A整;功正(){a:=123;形Println(整64(a));变b这A;变c这A;形Println(b,c)}`,
...
16:{`syntax "go";形Println("Hello, world!")`,
...
//================================================================================================================================================================
func TestGoSyntaxParseError(t *testing.T) {
	for i, tst:= range goSyntaxParseErrorTests {
		src:= tst.key
		dse:= tst.val
		fset := token.NewFileSet()
		/*defer func(){
...
var goSyntaxParseErrorTests = map[int]struct{key string; val string} {

// ========== ========== ========== ==========
//test invalid syntax specifier
1001:{`
syntax "go789"
package main
func main() {
...
// ---------- ---------- ---------- ----------
`suppliedName.gro:2:8: invalid syntax declaration: "go789"`},
...
// ---------- ---------- ---------- ----------
`suppliedName.gro:3:9: expected ';', found 'EOF'`},
