...
// Package parser implements a parser for Gro source files. Input may be
// provided in a variety of forms (see the various Parse* functions); the
// output is an abstract syntax tree (AST) representing the Gro source. The
// parser is invoked through one of the Parse* functions.
//
// The parser accepts a larger language than is syntactically permitted by
// the Gro spec, for simplicity, and for improved robustness in the presence
...
type parser struct {
	...
	// Additions to parser state for Unihan processing
	isInserted           bool // if a token been inserted into the stream inside the parser

	macroModes           map[rune]bool // tracks various macro modes
	unihanMode           bool // tracks lexically scoped Unihan-mode
	unihanOnLhs          string // tracks Unihan identifiers on left hand side
	prevIdentIsUnihan    bool // whether previous ident was Unihan

	currSource           string
	unihanImports        []*ast.ImportSpec // list of imports found during parsing
	unihanImportAliases  map[rune]string // map of Unihan aliases in import header
	...
	specialBinary        map[token.Token]func(macro.Parser, ast.Expr, ast.Expr)ast.Expr
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
func (p *parser) init(fset *token.FileSet, filename string, src []byte, aliases map[rune]macro.StmtMacro, mode Mode) {
	p.fset = fset
	...
	p.stmtMacros = map[rune]macro.StmtMacro{}
	for k, v:= range scanner.StmtMacros {
		p.stmtMacros[k] = v
		p.stmtMacros[k].Init(p)
	}
	if aliases != nil {
	...
	p.exprMacros = map[rune]macro.ExprMacro{}
	...
	p.specialUnary = map[token.Token]func(macro.Parser, ast.Expr)ast.Expr{}
	p.specialBinary = map[token.Token]func(macro.Parser, ast.Expr, ast.Expr)ast.Expr{}

	p.replVars = map[string]bool{}
	p.replConsts = map[string]bool{}

	p.Next()
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
func (p *parser) initSource(srcName string) {
	p.currSource = srcName

	p.unresolved = []*ast.Ident{}
	p.imports = []*ast.ImportSpec{}

	//p.comments = []*ast.CommentGroup{} //TODO: why not???
	p.targetStack = [][]*ast.Ident{}
	p.unihanImports = []*ast.ImportSpec{}
	p.unihanImportAliases = map[rune]string{}
	p.macroModes = map[rune]bool{}
}

// ----------------------------------------------------------------------------
// Added Unihan methods
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
func (p *parser) Pos() token.Pos { return p.pos }
func (p *parser) Tok() token.Token { return p.tok }
func (p *parser) SetTok(t token.Token) { p.tok = t }
func (p *parser) Lit() string { return p.lit }

func (p *parser) AppendUnihanImports(pis *ast.ImportSpec) {
	p.unihanImports = append(p.unihanImports, pis)
}

func (p *parser) AddSpecialProc(t token.Token, fn func(macro.Parser)ast.Expr) {
	p.specialProc[t]= fn
}

func (p *parser) AddSpecialUnary(t token.Token, fn func(macro.Parser, ast.Expr)ast.Expr) {
	p.specialUnary[t]= fn
}

func (p *parser) AddSpecialBinary(t token.Token, fn func(macro.Parser, ast.Expr, ast.Expr)ast.Expr) {
	p.specialBinary[t]= fn
}

func (p *parser) AddSpecialRuneProc(fn func(macro.Scanner) string) {
	p.scanner.AddSpecialRuneProc(fn)
}

func (p *parser) DeleteSpecialProc(t token.Token) { delete(p.specialProc, t) }
func (p *parser) DeleteSpecialUnary(t token.Token) { delete(p.specialUnary, t) }
func (p *parser) DeleteSpecialBinary(t token.Token) { delete(p.specialBinary, t) }

func (p *parser) DeleteSpecialRuneProc() {
	p.scanner.DeleteSpecialRuneProc()
}
...
func (p *parser) parseIdent() *ast.Ident {
	...
	if p.tok == token.IDENT {
		...
		if pkgLookup != "" {
			...
			name = pkgAlias
			p.insertToken(token.PERIOD, ".")
		} else if p.unihanMode && ! unicode.IsUpper(r) && name != "_" {
			name = "_" + name
		}
...
func (p *parser) parseFuncTypeOrLit() ast.Expr {
	...
	oldUnihanMode:= p.unihanMode
	if p.prevIdentIsUnihan && ! oldUnihanMode {
		p.unihanMode = true
		p.prevIdentIsUnihan = false
	}
...
func (p *parser) parseOperand(lhs bool) ast.Expr {
	...
	case token.IDENT:
		...
		if _, isMacro:= scanner.ExprMacros[r]; isMacro {
			...
			oldMacroMode:= p.macroModes[r]
			if ! oldMacroMode {
				p.macroModes[r] = true
			}
			s:= p.exprMacros[r].Main(p)
	...
	case token.INT, token.FLOAT, token.IMAG, token.CHAR, token.STRING:
		if p.specialProc[p.tok] != nil {
			return p.specialProc[p.tok](p)
		} else {
...
func (p *parser) parseUnaryExpr(lhs bool) ast.Expr {
	...
	case token.ADD, token.SUB, token.NOT, token.XOR, token.AND:
		...
		if p.specialUnary[op] != nil {
			return p.specialUnary[op](p, x)
		} else {
...
func (p *parser) parseBinaryExpr(lhs bool, prec1 int) ast.Expr {
	...
	for _, prec := p.tokPrec(); prec >= prec1; prec-- {
		for {
			...
			if p.specialBinary[op] != nil {
				x = p.specialBinary[op](p, x, y)
			} else {
...
func (p *parser) parseSimpleStmt(mode int) (ast.Stmt, bool) {
	...
	case
		...
		if p.tok == token.DEFINE && p.replMode {
			for _, n:= range x {
	...
	case
		...
		if p.tok == token.IDENT && p.lit == "让" {
			...
			p.AppendUnihanImports(&ast.ImportSpec{
				Name: &ast.Ident{NamePos: token.NoPos, Name: "assert"},
				Path: &ast.BasicLit{ValuePos: token.NoPos, Kind: token.STRING, Value: "github.com/grolang/gro/assert"},
			})
			x:= p.parseRhs()
			s = &ast.ExprStmt{
				/*`assert.AssertTrue(` + x + `)`*/
				X: &ast.CallExpr {
					Fun: &ast.SelectorExpr { X: &ast.Ident { Name: "assert" }, Sel: &ast.Ident { Name: "AssertTrue" } },
					Args: []ast.Expr { x },
				},
			}
...
func (p *parser) parseImportSpec(doc *ast.CommentGroup, _ token.Token, _ int) ast.Spec {
	...
	outer: for r, _:= utf8.DecodeRune([]byte(p.lit));
		...
		p.error(p.pos, "unexpected Unihan: " + p.lit)
	}
	...
	if p.tok == token.STRING {
		...
		if p.unihanMode && ident == nil {
			ident = &ast.Ident{NamePos: oldPos, Name: "_" + filepath.Base(path[1:len(path)-1])}
		}

		for _, a:= range aliases {
			if ident == nil || ident.Name == "" {
				p.unihanImportAliases[a] = filepath.Base(path[1:len(path)-1])
			} else {
				p.unihanImportAliases[a] = ident.Name
			}
		}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
func (p *parser) parseGenDecl(keyword token.Token, f parseSpecFunction, oldUnihanMode bool) *ast.GenDecl {
	if p.trace {
...
// Source files
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
func (p *parser) parseSource(doc *ast.CommentGroup, pos token.Pos, ident *ast.Ident) *ast.File {
	if p.trace {
	...
	var mainIdx = -1
	var stmts []ast.Stmt //standalone stmts
			...
			for p.tok != token.EOF && p.lit != "源" && p.tok != token.PACKAGE {
				...
				case
					...
					token.SEMICOLON, token.RBRACE: // terminators
						...
						if _, isLabeledStmt := s.(*ast.LabeledStmt); isLabeledStmt {
							pos := p.pos
							p.ErrorExpected(pos, "declaration or unlabelled statement")
							syncStmt(p)
							s = &ast.BadStmt{From: pos, To: p.pos}
						}
				...
				default:
					...
					syncStmt(p)
					stmts = append(stmts, &ast.BadStmt{From: pos, To: p.pos})
				}
			}
		}
	}
	...
	// append stmts to func main() body
	if len(stmts) > 0 {
		if funcMain == nil {
			funcMain = &ast.FuncDecl{
				Doc:  &ast.CommentGroup{},
				Recv: nil,
				Name: &ast.Ident{NamePos: token.NoPos, Name: "main"},
				Type: &ast.FuncType{
					Func:    token.NoPos,
					Params:  &ast.FieldList{Opening: token.NoPos, List: []*ast.Field{}, Closing: token.NoPos},
					Results: &ast.FieldList{Opening: token.NoPos, List: []*ast.Field{}, Closing: token.NoPos},
				},
				Body: &ast.BlockStmt{Lbrace: token.NoPos, List: []ast.Stmt{}, Rbrace: token.NoPos},
			}
		}
		for _, s:= range stmts {
			funcMain.Body.List = append(funcMain.Body.List, s)
		}
	}
	if funcMain != nil && mainIdx == -1 {
		decls = append(decls, funcMain)
	}
	...
	for _, ident := range p.unresolved {
		...
		assert(ident.Obj == unresolved, "object already resolved" + fmt.Sprint(ident))
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
func (p *parser) parseUseSpec(doc *ast.CommentGroup) (imports, vars string) {
	if p.trace {
		defer un(trace(p, "UseSpec"))
	}
	...
	if p.tok == token.STRING {
		path = p.lit
		if !isValidImport(path) {
			p.error(pos, "invalid use path: "+path)
		}
		ident = &ast.Ident{NamePos: p.pos, Name: "_" + filepath.Base(path[1:len(path)-1])}
		p.Next()
	} else {
		p.expect(token.STRING) // use expect() error handling
	}
	...
	imports = doc.Text()
	if imports != "" { imports += "\n" }
	imports += ident.String() + " " + path
	if p.lineComment.Text() != "" { imports += " //" + p.lineComment.Text() }
	imports += "\n"

	for _, a:= range aliases {
		vars += "'" + string(a) + "': " + ident.Name + ".Struct{},\n"
	}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
func (p *parser) parseUseDecl() string {
	if p.trace {
	...
	if p.tok == token.LPAREN {
		...
		for ; p.tok != token.RPAREN && p.tok != token.EOF; count++ {
			is, vs:= p.parseUseSpec(p.leadComment)
			imports += is
			vars += vs
		}
		...
		is, vs:= p.parseUseSpec(nil)
		imports += "\t" + is
		vars += "\t" + vs
		count++
	}
	p.unihanMode = false //TODO: do we need this?
	if count == 0 {
		return ""
	} else {
		pgm:= `package main
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
func (p *parser) parseMultiFile() map[string]*ast.File {
	if p.trace {
		defer un(trace(p, "MultiFile"))
	}

	// Don't bother parsing the rest if we had errors scanning the first token.
	// Likely not a Gro source file at all.
	if p.errors.Len() != 0 {
	...
	// Don't bother parsing the rest if we had errors parsing the use declarations.
	if p.errors.Len() != 0 {
		return map[string]*ast.File{}
	}
	...
	for p.tok != token.EOF {
		...
		if p.tok == token.PACKAGE {
			...
			if dirName != "" {
				if pkgName == "" {
					ident.Name = filepath.Base(dirName)
				}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
func (p *parser) parseRepl() map[string]*ast.File {
	if p.trace {
		defer un(trace(p, "Repl"))
	}

	srcs:= map[string]*ast.File{}
	p.replMode = true

	if p.tok == token.EOF && p.replMode {
		p.replState.CmdFound = NextCmd
		return srcs
	}
	...
	for p.tok != token.EOF {
		doc := p.leadComment
		pos := token.NoPos + 1
		ident := &ast.Ident{NamePos: token.NoPos, Name: "main"}
		...
		for p.tok != token.EOF {
			...
			outp:= "\"var(\\n"
			for r, _:= range p.replVars {
				outp = outp + r + "=\" + 形Sprintf(\"%#v\", " + r + ") + \"\\n"
			}
			outp = outp + ")\\n"
			outp = outp + "const(\\n"
			for r, _:= range p.replConsts {
				outp = outp + r + "=\" + 形Sprintf(\"%#v\", " + r + ") + \"\\n"
			}
			outp = outp + ")\\n\""
			p.replState.VarFragment = "_= ioutil.WriteFile(\"tmp/repl-var.gro\", []byte(" + outp + "), os.ModeExclusive)"
			p.replState.ImpFragment = "import(\n\t\"io/ioutil\"\n\t\"os\"\n)\n"
		}
	}
	p.replMode = false
	return srcs
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
func (p *parser) parseUsesOnly() string {
	if p.trace {
		defer un(trace(p, "Uses"))
	}
