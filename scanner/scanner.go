...
// Package scanner implements a scanner for Gro source text.
...
type Scanner struct {
	...
	// Additions to scanner state for Unihan processing
	specialRuneProc func(macro.Scanner) string
}
...
func (s *Scanner) AddSpecialRuneProc (f func (macro.Scanner) string) {
	s.specialRuneProc = f
}

func (s *Scanner) DeleteSpecialRuneProc() {
	s.specialRuneProc = nil
}
...
func isNonUnihanLetter(ch rune) bool {
...
func (s *Scanner) scanIdentifier() string {
	offs := s.offset
	for isNonUnihanLetter(s.ch) || isDigit(s.ch) {
		s.Next()
	}
	return string(s.src[offs:s.offset])
}

func (s *Scanner) scanUnihan() rune {
	offs := s.offset
	s.Next()
