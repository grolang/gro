...
func StringWithLinebreaks(n Node) string {
	var buf bytes.Buffer
...
func impliesSemi(tok token) bool {
	switch tok {
	case _Name,
		_Break, _Continue, _Fallthrough, _Return, _Comment,
...
//================================================================================
// private methods of printer beginning with "print"
//--------------------------------------------------------------------------------
...
func (p *printer) printNode(n Node) {
	...
	if ncom != nil && len(ncom.Right) > 0 {
		for i, c := range ncom.Right {
			if i+1 < len(ncom.Right) {
				if c.Text == "" || lineComment(c.Text) {
					panic("unexpected empty line or //-style non-final 'right' comment")
				}
			}
			p.print(blank, c)
		}
	}
	if ncom != nil && len(ncom.Below) > 0 {
		for _, c := range ncom.Below {
			if c.Text == "" {
				panic("unexpected empty line")
			}
			p.print(newline, c)
...
func (p *printer) printDecl(list []Decl) {
	...
	if _, ok := list[0].(*EmptyDecl); ok { //CODE FOR COMMENTS
		if len(list) != 1 {
			panic("unreachable")
		}
	...
	var pg printGroup
	// *pg.Comments() = *group.Comments() //CODE FOR COMMENTS
	pg.Tok = tok
	pg.Decls = list
	p.printNode(&pg)
}
